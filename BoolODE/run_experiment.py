...
def simulateAndSample(argdict):
    ...
    index = [i for i,n in varmapper.items()]    
    tnamepname = [n for i,n in varmapper.items()]
    ## Velocity id's 
    index     = index +     [len(index) + i for i in index]
    tnamepname = tnamepname + ["velocity_" + name for name in tnamepname]
    ...
    while retry:
        ...
        retry = False
        ## Extract Time points
        # These lines were changed to output protein levels and velocities too
        subset = P[index,:][:,tps]        
